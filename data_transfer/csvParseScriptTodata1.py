        ...
        for j in range((sheet.nrows)-1):
            ...
            if rowsx+1 < sheet.nrows:
                ...
                if (tempBlock != tb  or tempYear != ty): # check next row  
                    ...
                    with open('dataOutput1/' + 'run'+ str(fileN)+'.json', 'a') as json_file:
...
#with open('parsedData.json', 'w') as json_file:
 #   json.dump(outer, json_file)
  #  json_file.write("\n")
xlsxTocsv()
path, dirs, files = next(os.walk('dataOutput1/'))
for item in os.listdir('dataOutput1/'):
    if not item.startswith('.') and os.path.isfile(os.path.join('dataOutput1/', item)):
        print (item)